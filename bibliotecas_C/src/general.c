...
int grl_Count_Letters(char* string)
{
	int counter;
	int i;
	int len;
	...
	if(string != NULL)
	{
		...
		for(i = 0; i < len; i++ )
		{
			if(isalpha(*(string+i)) || *(string+i) == 'ñ' || *(string+i) == 'Ñ')
			{
				counter ++;
			}
		}
...
int grl_Get_ValidStingLettersWithSpaces(char* string, int len, char* message, char* errorMessage)
{
	...
	if(string != NULL)
	{
		...
		do
		{
			...
			if(!grl_Get_StringLetttersWithSpaces(string, len) && grl_Count_Letters(string) > 2)
			{
...
int grl_Get_ValidName(char* string, int len, char* message, char* errorMessage)
{
	...
	if(string != NULL)
	{
		...
		do
		{
			...
			{
				if(grl_Count_Letters(string) >= min)
				{
